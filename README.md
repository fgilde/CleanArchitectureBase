...
This is a solution a Single Page App (SPA) with Angular and ASP.NET Core following the principles of Clean Architecture. 
...
3. Navigate to `src/WebAPI/ClientApp` and run `npm install`
4. Navigate to `src/WebAPI/ClientApp` and run `npm start` to launch the front end (Angular)
5. Navigate to `src/WebAPI` and run `dotnet run` to launch the back end (ASP.NET Core Web API) or open Solution in Visual Studio and launch WebAPI

As alternative you can navigate to `src` and run `start.bat`
